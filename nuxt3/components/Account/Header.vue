    ...
    <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
      ...
      <div class="flex items-center gap-x-4 lg:gap-x-6">
        <BaseDropdown
          ...
          :options="spaces"
          @update:model-value="currentSpace"
        />
        <BaseButton
          id="add-new-space"
          class="p-0"
          :type="buttonTypes.BUTTON"
          @click="openNewSpace = true"
          ><PlusIcon class="h-6 text-gray-400" aria-hidden="true"
        /></BaseButton>
        <Modal
          :open="openNewSpace"
          title="Add A New Space"
          @close="openNewSpace = false"
        >
          <ModalNewSpace />
        </Modal>
        <BaseColorMode />
        <BaseButton
          id="notification-btn"
          :type="buttonTypes.BUTTON"
          class="-m-2.5 p-2.5 text-gray-400 dark:text-gray-300 hover:text-gray-500 bg-transparent hover:bg-transparent"
...
import { buttonTypes, selectOptions } from "@/types";

const spaceStore = useSpaceStore();

const spaces = computed(() => {
  const spaceOptions: selectOptions = [];
  spaceStore.getSpaces?.forEach((space) => {
    spaceOptions.push({ value: space.id, label: space.name });
  });

  return spaceOptions;
});

const selectedSpaceId = ref(spaceStore.getCurrentSpaceId);

const currentSpace = () => {
  const selectedSpace = spaceStore.getSpaces?.find(
    (spaceItem) => spaceItem.id === selectedSpaceId.value,
  );
  if (selectedSpace) {
    return spaceStore.setCurrentSpace(selectedSpace);
  }
};
